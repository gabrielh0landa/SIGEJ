...
class ConsultaService {
    ...
    '''
        ...
        // Converter java.util.Date para java.sql.Date ou java.sql.Timestamp
        def params = [
                inicio: new java.sql.Timestamp(dataInicio.getTime()),
                fim: new java.sql.Timestamp(dataFim.getTime())
        ]
    ...
    List<GroovyRowResult> ordensConcluidasPorTipo(Integer ano) {
        String sql = '''
            SELECT tos.descricao AS tipo_servico, 
                   ...
                   ROUND(AVG(EXTRACT(EPOCH FROM (aos.fim_atendimento - aos.inicio_atendimento))/3600), 2) AS tempo_medio_horas
              FROM ordem_servico os
              JOIN tipo_ordem_servico tos ON os.tipo_os_id = tos.id
              JOIN status_ordem_servico sts ON os.status_id = sts.id
              LEFT JOIN andamento_ordem_servico aos ON os.id = aos.os_id 
                   AND aos.status_novo_id = (SELECT id FROM status_ordem_servico WHERE descricao = 'Concluída')
    ...
    List<GroovyRowResult> relatorioOrdensServico(Map filtros) {
        ...
        if (filtros.dataInicio && filtros.dataFim) {
            ...
            params.inicio = new java.sql.Date(filtros.dataInicio.getTime())
            params.fim = new java.sql.Date(filtros.dataFim.getTime())
        }
